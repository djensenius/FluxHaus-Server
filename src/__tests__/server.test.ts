...
describe('Server', () => {
  ...
  beforeEach(async () => {
    ...
    process.env.ROBOT_CONNECTION_TYPE = 'direct';
    ...
    (Robot as unknown as jest.Mock).mockImplementation((config) => {
      if (config.name === 'Broombot') return mockBroombot;
      return mockMopbot;
    });
    // Mock static methods
    /* eslint-disable @typescript-eslint/no-explicit-any */
    (Robot as any).batteryLevelStatus = jest.fn().mockReturnValue(100);
    (Robot as any).binStatus = jest.fn().mockReturnValue('OK');
    (Robot as any).runningStatus = jest.fn().mockReturnValue(false);
    (Robot as any).chargingStatus = jest.fn().mockReturnValue(false);
    (Robot as any).dockingStatus = jest.fn().mockReturnValue(false);
    (Robot as any).dockedStatus = jest.fn().mockReturnValue('Docked');
    (Robot as any).batteryStatus = jest.fn().mockReturnValue('Normal');
    /* eslint-enable @typescript-eslint/no-explicit-any */
